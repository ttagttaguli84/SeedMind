...
| ~~PATTERN-001~~ | ~~-~~ | ~~[self-improve 전용] 신규 문서가 canonical 문서의 수치/이름을 복사해 독립 기재 → 불일치 반복 발생~~ — DONE → `.claude/rules/doc-standards.md` Canonical 데이터 매핑 섹션 추가 (`docs/reports/self_improve_20260406_v2.md`) |
| ~~PATTERN-002~~ | ~~-~~ | ~~[self-improve 전용] 동일 문서 내에서도 섹션 간 수치 불일치 발생~~ — DONE → `.claude/rules/workflow.md` Reviewer Checklist 항목 1~4 추가 (`docs/reports/self_improve_20260406_v2.md`) |
| ~~PATTERN-003~~ | ~~-~~ | ~~[self-improve 전용] enum/타입 확장 시 같은 문서 내 다른 섹션(pseudo-code, 예시 코드 등)이 업데이트되지 않음~~ — DONE → `.claude/rules/doc-standards.md` Consistency Rules에 enum 전수 업데이트 규칙 추가, `.claude/rules/workflow.md` Reviewer Checklist 항목 5 추가 (`docs/reports/self_improve_20260406_v2.md`) |
| ~~PATTERN-004~~ | ~~-~~ | ~~[self-improve 전용] 같은 문서 내 디자인 섹션(Part I)과 MCP 구현 섹션(Part II) 간 불일치 반복 발생 (Buildings 부모, 시작 시각, EconomyManager 배치, TestPlayer 누락 등 4건)~~ — DONE → `.claude/rules/workflow.md` Reviewer Checklist 항목 6 추가 (`docs/reports/self_improve_20260406_v2.md`) |
...
| ~~DES-021~~ | ~~1~~ | ~~보조 XP 소스 합산 비중 상한 기준 설계 원칙 문서화 (BAL-019 OPEN — 보조 소스 51.2%, 60% 초과 시 재검토 기준 명시)~~ — DONE: xp-integration.md 섹션 7 추가 (soft cap 60%, Yellow Zone 55~60%, 조치 우선순위 3단계), bal-019-xp-balance.md OPEN → RESOLVED:DES-021 |
...
| ~~FIX-105~~ | ~~1~~ | ~~collection-system.md 섹션 6.1 단축키 바인딩 OPEN#2 결정 (fish-catalog.md 섹션 5.1과 동일 이슈 — UI 시스템 단축키 정책 통일, ui-system.md 섹션 7 참조)~~ — DONE: `C` 키로 확정. collection-system.md 섹션 6.1/OPEN#2, fish-catalog.md 섹션 5.1/OPEN#3, ui-system.md 섹션 11 동시 업데이트 |
...
| ~~ARC-044~~ | ~~1~~ | ~~전체 MCP 태스크 시퀀스 의존성 그래프 문서화 — 각 Phase가 선행하는 Phase를 명시한 빌드 순서 개요 작성 (Phase 2 구현 착수 시 실행 순서 로드맵)~~ — DONE → `docs/mcp/build-order.md` (7 Phase A~G, 의존성 그래프, 크리티컬 패스, tutorial-tasks Phase 분류 오류 수정) |
...
| ~~PATTERN-011~~ | ~~-~~ | ~~[self-improve 전용] MCP 태스크 문서 예시 에셋명이 canonical 콘텐츠 문서 영문 ID와 불일치하는 패턴 반복 발생 (ARC-046에서 Fence/Path/Light 3개 카테고리 3건 — FenceBrick/PathCobble/PathFlower/LightFairy 등 임의 생성). canonical SO ID를 직접 조회하지 않고 임의 작성 → MCP 태스크 작성 시 에셋명은 반드시 canonical 콘텐츠 문서에서 조회하도록 규칙화 필요~~ — DONE: doc-standards.md PATTERN-011 규칙 추가, workflow.md Reviewer Checklist 항목 15 추가 |
| ~~DES-024~~ | ~~2~~ | ~~에너지 시스템 상세 설계 — 일일 에너지 최대값, 행동별 소모량 통합 테이블, 회복 방식(수면/음식), 에너지 고갈 패널티 확정. farming/fishing/gathering에 분산 기재된 에너지 규칙을 `docs/systems/energy-system.md`로 통합~~ — DONE → `docs/systems/energy-system.md` (최대 에너지 100, 행동별 소모 통합 테이블, 수면/음식 회복, 고갈 패널티, 날씨/시간대 배수, 튜닝 파라미터 canonical 확정) |
| ~~CON-020~~ | ~~1~~ | ~~장식 아이템 콘텐츠 상세 — decoration-system.md 5개 카테고리(울타리/경로/조명/장식물/수경) 전체 아이템 목록·가격·배치 규칙 정의 (`docs/content/decoration-items.md` 신규, decoration-architecture.md SO 에셋 생성 전 필요)~~ — DONE → `docs/content/decoration-items.md` (29종 itemId/buyPrice/타일/파라미터 canonical 확정, OPEN#A 복수 시즌 이슈 플래그) |
| ~~ARC-044~~ | ~~2~~ | ~~에너지 시스템 기술 아키텍처 (EnergyManager, EnergyConfig SO, EnergyEvents, EnergyBarUI, 세이브/로드 통합 — DES-024 후속)~~ — DONE → `docs/systems/energy-architecture.md` (SaveLoadOrder 51 확정, data-pipeline.md EnergyConfig SO 추가) |
| ~~FIX-118~~ | ~~2~~ | ~~farming-system.md / fishing-system.md / gathering-system.md / time-season.md 에너지 소모 수치를 `energy-system.md` canonical 참조로 교체 (DES-024 후속 — 분산 기재 에너지 수치가 아직 원본 문서에 잔존)~~ — DONE: 4개 문서 섹션 내 직접 수치 → canonical 참조 교체, Cross-references 갱신 |
| ~~ARC-047~~ | ~~1~~ | ~~에너지 시스템 MCP 태스크 시퀀스 독립 문서화 (energy-architecture.md Part II 개요 → docs/mcp/energy-tasks.md 상세 분리 — ARC-044 후속)~~ — DONE → `docs/mcp/energy-tasks.md` (7개 태스크 그룹 E-A~E-G, ~55회 MCP 호출) |
...
| ~~ARC-050~~ | ~~1~~ | ~~작물 성장 시스템 MCP 태스크 시퀀스 독립 문서화 (crop-growth-architecture.md Part II 요약 → docs/mcp/crop-growth-tasks.md 상세 분리)~~ — DONE → `docs/mcp/crop-growth-tasks.md` (3개 태스크 그룹 CG-A~C, ~79회 MCP 호출), crop-growth-architecture.md 섹션 1 클래스 다이어그램/섹션 9 수치 오류 수정 |
| ~~DES-012~~ | ~~2~~ | ~~플레이어 캐릭터 시스템 설계 (이동 속도/타일 인터랙션 반경/애니메이션 상태 머신/카메라 추적 방식 — scene-setup-tasks.md에 분산된 내용 통합 + 미정 항목 확정)~~ — DONE → `docs/systems/player-character.md` (이동 4 tiles/sec, 인터랙션 반경 1.5 tiles, 7개 애니메이션 상태, 쿼터뷰 Cinemachine 확정) |
| ~~FIX-120~~ | ~~1~~ | ~~crop-growth-architecture.md 섹션 4.3 seasonBonus 수치 충돌 해소 — 선언 범위(1.0~1.1)와 표 내 여름+과일류(1.2) 불일치, 온실 겨울(0.8)과 crop-growth.md 섹션 2.4 온실 비계절(x0.85) 불일치 → crop-growth.md 섹션 2.4 canonical 기준으로 표 교체 및 참조 표기~~ — DONE: 섹션 4.3 seasonBonus 범위 → canonical 참조, 계절 보너스 표 7행으로 교체 (여름 ×1.05 추가 적용, 온실 비계절 ×0.85, 겨울 전용 ×1.0 명시) |
