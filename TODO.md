...
| BAL-024 | 2 | 낚시+채집 합산 비율 구조적 이탈 분석 및 조정안 검토 — BAL-023 리뷰에서 새 기준(60%)도 시나리오 B(72%)·C(65%)에서 초과 확인. 낚시 에너지 비용 상향 또는 일일 낚시 시간 제약 도입 중 결정 (economy-system.md 섹션 8 [RISK] 낚시 우위 구조 후속) |
...
| FIX-111 | 2 | save-load-architecture.md GameSaveData JSON/C# 클래스에 `decoration: DecorationSaveData` 필드 추가 (ARC-043 후속 — PATTERN-005 준수) |
