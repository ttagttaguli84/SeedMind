...
- **(PATTERN-011) Asset names/SO IDs in MCP task documents must be looked up from canonical content documents**: When writing an MCP task document, every asset name and SO ID (e.g. decoration item IDs, facility names, crop IDs) must be copied verbatim from the relevant canonical content document (e.g. `docs/content/decoration-items.md`, `docs/content/facilities.md`, `docs/design.md`). Inventing asset names without looking them up is prohibited — arbitrary names cause import-time reference errors in Unity. Lookup procedure: open the canonical document, find the exact `itemId`/`soName`/English ID field, and copy it character-for-character into the MCP task. The reviewer verifies this through Reviewer Checklist item 15.
