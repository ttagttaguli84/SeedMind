...
| 가공 레시피 | ProcessingRecipeData | ~15 | 가공 유형별 배수/시간 | 본 문서 섹션 2.5 (신규) |
| 레벨 | LevelConfig | 1 | 레벨별 경험치/해금 설정 | 본 문서 섹션 2.6 (신규) |
...
| upgradeGoldCost | int | 0 | 업그레이드 골드 비용 (-> see `docs/systems/farming-system.md` 섹션 7.1) |
| upgradeTimeDays | int | 0 | 업그레이드 소요 일수 (-> see `docs/systems/farming-system.md` 섹션 7.1) |
| specialEffect | string | "" | 등급별 특수 효과 설명 (-> see `docs/systems/farming-system.md` 섹션 7.2) |
...
```
SO_Tool_Hoe_Basic  (tier=1, nextTier=SO_Tool_Hoe_Copper)
SO_Tool_Hoe_Copper (tier=2, nextTier=SO_Tool_Hoe_Iron)
SO_Tool_Hoe_Iron   (tier=3, nextTier=SO_Tool_Hoe_Gold)
SO_Tool_Hoe_Gold   (tier=4, nextTier=SO_Tool_Hoe_Iridium)
SO_Tool_Hoe_Iridium(tier=5, nextTier=null)
```

도구 5종(호미, 물뿌리개, 씨앗봉투, 낫, 손) 중 업그레이드 가능한 것은 호미, 물뿌리개, 낫 3종 x 5등급 = 15개. 씨앗봉투, 손은 각 1개 = 총 17개 에셋. 하지만 일부 도구의 등급 분화를 고려하면 최대 22개까지 가능.
...
| 필드 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| buildingId | string | "" | 코드용 고유 식별자 (예: "greenhouse") |
| buildingName | string | "" | 표시 이름 (예: "온실") |
...
| SO_Bldg_Greenhouse | greenhouse | (4,4) | SeasonBypass | 0 (내부만) | 3 |
| SO_Bldg_Storage | storage | (2,3) | Storage | 0 | 2 |
| SO_Bldg_Processor | processor | (2,2) | Processing | 0 | 3 |

**물탱크 상세**: effectRadius=3은 물탱크 중심에서 3타일 이내의 모든 경작 타일에 매일 아침 자동으로 Watered 상태를 부여한다. 물뿌리개로 물을 줄 필요가 없어진다.

**온실 상세**: tileSize (4,4) = 내부 경작 가능 영역은 테두리를 제외한 2x2 = 4타일. SeasonBypass 효과로 겨울에도 모든 작물 재배 가능.

[OPEN] 온실 내부 경작 가능 영역이 4타일로 충분한지. 초기에는 작으나 업그레이드로 확장(6x6 -> 4x4 내부) 가능성 검토 필요.
...
플레이어 레벨/경험치 시스템의 설정을 정의하는 ScriptableObject.
...
| 레벨 | 누적 경험치 | 구간 경험치 | 해금 내용 |
|------|-----------|-----------|-----------|
| 1 | 0 | 0 | 게임 시작 (감자, 당근) |
| 2 | 50 | 50 | 토마토 해금 |
| 3 | 150 | 100 | 옥수수, 고급 비료, 물탱크 해금 |
| 4 | 350 | 200 | 딸기, 속성장 비료, 창고 해금 |
| 5 | 650 | 300 | 호박, 해바라기, 온실 해금 |
| 6 | 1100 | 450 | 유기 비료 해금 |
| 7 | 1700 | 600 | 수박, 가공소 해금 |
| 8 | 2500 | 800 | (향후 콘텐츠) |
| 9 | 3500 | 1000 | (향후 콘텐츠) |
| 10 | 5000 | 1500 | 최종 레벨 |
...
**예시**: 토마토(5일 성장) 금별 품질 수확

```
기본 경험치 = 5 + floor(5 * 1.0) = 10
품질 보너스 = qualityExpBonus[Gold] = 1.5
최종 경험치 = 10 * 1.5 = 15 EXP
```
...
**해금 판정 로직**: 레벨업 시 LevelConfig의 해금 테이블을 참조하여 새로 해금된 항목을 UnlockSaveData에 추가한다. 한 번 해금된 항목은 영구적이다.
...
| LevelConfig | maxLevel, expPerLevel | (모든 필드 필수) |
...
| ToolData.tier | 1 | 5 | 5등급(이리듐)이 최고 |
...
| LevelConfig.maxLevel | 1 | 100 | 현재 설계는 10 |
...
| 밸런스 포인트 | 데이터 원천 | 조정 방법 |
|-------------|-----------|----------|
| 레벨업 속도 | LevelConfig.expPerLevel, harvestExpBase | 경험치 테이블 조정 |
...
│
├── Tools/                                     # 도구 데이터
│   ├── SO_Tool_Hoe_T1.asset ~ T5
│   ├── SO_Tool_WateringCan_T1.asset ~ T5
│   ├── SO_Tool_Sickle_T1.asset ~ T5
