...
| 장식품 | DecorationItemData | 29 | 장식품별 카테고리/가격/배치 방식 데이터 (ARC-043) | `docs/systems/decoration-architecture.md` 섹션 2.2 |
| 장식 설정 | DecorationConfig | 1 | 장식 시스템 전역 설정 (ARC-043) | `docs/systems/decoration-architecture.md` 섹션 2.3 |
| 에너지 설정 | EnergyConfig | 1 | 에너지 시스템 튜닝 파라미터 (ARC-044) | `docs/systems/energy-architecture.md` 섹션 3 |

**총 예상 에셋 수**: ~248개 SO 에셋 (ARC-033 +33, ARC-045 GatheringCatalogData 27, ARC-043 DecorationItemData 29+DecorationConfig 1, ARC-044 EnergyConfig 1 추가, 갱신)
...
| energyCost | int | 1 | 사용 당 에너지 소모 (-> see `docs/systems/energy-system.md` 섹션 2.1) |
...
#### 2.14 DecorationItemData (신규, ARC-043)

개별 장식품 데이터를 정의하는 ScriptableObject. `ScriptableObject`를 직접 상속한다. 카테고리별(Fence/Path/Light/Ornament/WaterDecor) 파라미터를 단일 클래스에 포함하며, 미사용 필드는 0/null로 유지한다.

(-> see `docs/systems/decoration-architecture.md` 섹션 2.2 for canonical 클래스 정의)

| 필드 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| itemId | string | "" | 장식품 고유 ID (예: "FenceStone") |
| displayName | string | "" | UI 표시명 |
| icon | Sprite | null | 인벤토리/상점 아이콘 (Inspector 할당) |
| category | DecoCategoryType | Fence | 장식 카테고리 (-> see `docs/systems/decoration-system.md` 섹션 1.2) |
| buyPrice | int | 0 | 구매 가격 (-> see `docs/content/decoration-items.md` 섹션 1~5) |
| isEdgePlaced | bool | false | true = 울타리(edge 배치), false = 타일 점유 |
| tileWidthX | int | 1 | 점유 타일 X (isEdgePlaced=true이면 0) |
| tileHeightZ | int | 1 | 점유 타일 Z (isEdgePlaced=true이면 0) |
| unlockLevel | int | 0 | 해금 필요 레벨; 0 = 시작부터 가능 (-> see `docs/content/decoration-items.md` 섹션 1~5) |
| unlockZoneId | string | "" | 해금 구역 ID; 없으면 "" (-> see `docs/systems/farm-expansion.md` 섹션 1.3) |
| limitedSeason | Season | None | 판매 계절 제한; None = 항상 판매 (-> see `docs/systems/time-season.md` 계절 정의) |
| lightRadius | float | 0 | 조명 반경 — Light 전용 (-> see `docs/content/decoration-items.md` 섹션 3.1) |
| moveSpeedBonus | float | 0 | 이동속도 보너스 비율 — Path 전용 (-> see `docs/content/decoration-items.md` 섹션 2.1) |
| durabilityMax | int | 0 | 내구도 최대값 — Fence 전용; 0 = 영구 (-> see `docs/content/decoration-items.md` 섹션 1.1) |
| prefab | GameObject | null | 오브젝트 프리팹 — Light/Ornament/WaterDecor 전용 |
| floorTile | TileBase | null | Tilemap 타일 — Path 전용 |
| edgeTileH | TileBase | null | 울타리 수평 스프라이트 — Fence 전용 |
| edgeTileV | TileBase | null | 울타리 수직 스프라이트 — Fence 전용 |
| edgeTileCorner | TileBase | null | 울타리 코너 스프라이트 — Fence 전용 |
| description | string | "" | 장식품 설명 텍스트 |

buyPrice, unlockLevel, lightRadius, moveSpeedBonus, durabilityMax 등 **콘텐츠 수치는 `docs/content/decoration-items.md` (CON-020) 섹션 1~5에서만 정의**한다. 이 테이블에 직접 기재하지 않는다 (PATTERN-007).

**에셋 목록**: 총 29종 (Fence 4 + Path 5 + Light 4 + Ornament 11 + WaterDecor 5). 에셋명 패턴: `SO_Deco_<ID>` (예: `SO_Deco_FenceStone`). 카테고리별 에셋 수: (-> see `docs/systems/decoration-architecture.md` 섹션 2.2 SO 에셋 데이터 테이블)

---

#### 2.15 DecorationConfig (신규, ARC-043)

장식 시스템의 전역 설정을 정의하는 ScriptableObject. 게임 전체에서 1개 에셋만 존재한다.

(-> see `docs/systems/decoration-architecture.md` 섹션 2.3 for canonical 클래스 정의)

| 필드 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| validHighlightColor | Color | 초록 | 배치 가능 하이라이트 색 |
| invalidHighlightColor | Color | 빨강 | 배치 불가 하이라이트 색 |
| fenceDurabilityDecayPerSeason | float | 0 | 울타리 계절당 내구도 감소율 (-> see `docs/content/decoration-items.md` 섹션 1.2) |
| fenceRepairCostRatio | float | 0 | 수리 비용 = buyPrice × repairRatio (-> see `docs/content/decoration-items.md` 섹션 1.1) |
| pathSpeedBonusEnabled | bool | true | Path 이동속도 보너스 활성화 여부 (-> see `docs/content/decoration-items.md` 섹션 2.1) |

fenceDurabilityDecayPerSeason, fenceRepairCostRatio 등 **콘텐츠 수치는 `docs/content/decoration-items.md` (CON-020)에서만 정의**한다 (PATTERN-007).
