...
[RISK] URP Pipeline Asset 사전 생성: URP는 Pipeline Asset(SO)이 Graphics Settings에 할당되어야 동작한다. MCP로 할당이 불가능할 경우 수동 1회 설정 후 진행.

[RISK] MCP 도구 가용성: MCP for Unity의 정확한 도구 목록은 버전에 따라 다를 수 있다. Phase A 시작 전 사용 가능 도구를 확인하고 매핑 조정 필요.
...
Step A-2-01: execute_menu_item -> "Assets/Create/Rendering/URP Asset (with Universal Renderer)"
             [RISK] MCP로 생성된 에셋의 이름/경로 제어가 어려울 수 있음. 수동 대체 가능.
Step A-2-02: Graphics Settings에 URP Pipeline Asset 할당
             [RISK] 프로젝트 설정 변경이 MCP로 가능한지 미확인.
...
Step A-3-01: SeedMindInputActions.inputactions 생성
             ...
             [RISK] Input Actions 내부 액션 맵은 .inputactions JSON 직접 작성이 더 신뢰성 높음.
```
...
Step B-8-08: create_object -> name: "AmbientProbe", parent: "Lighting"
             [RISK] RenderSettings.ambientLight 변경이 MCP로 가능한지 미확인.
```
...
Step B-9-01: Main Camera를 "--- CAMERA ---" 하위로 이동
             [RISK] 씬 생성 방식에 따라 기본 카메라가 포함되지 않을 수 있음. 존재 여부 확인 후 조건부 실행.
...
[RISK] Build Settings 편집이 MCP로 불가능할 수 있음. 대안: EditorBuildSettings.scenes 설정 Editor 스크립트 실행.
...
- [RISK] URP Pipeline Asset을 MCP로 생성/할당 불가 -> 수동 1회 설정.
- [RISK] RenderSettings(Ambient Light 등) MCP 접근 불가 -> Lighting Settings 수동 조정.
- [RISK] Build Settings 편집 MCP 미지원 -> Editor 스크립트 대안.
- [RISK] Input Actions 내부 액션 맵 MCP 편집 불가 -> .inputactions JSON 직접 작성.
- [RISK] Canvas 컴포넌트 설정 시 MCP 프로퍼티 경로 불일치 -> get_object_info로 사전 확인.
- [RISK] 씬 생성 시 기본 포함 오브젝트(Camera, Light)가 버전별 상이 -> 생성 직후 기존 오브젝트 목록 조회.
- [RISK] MCP 도구명/파라미터가 실제 설치 버전과 불일치 -> Phase A 시작 전 도구 목록 검증 필수.
