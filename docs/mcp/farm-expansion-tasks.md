...
> **[최적화 필수]** Z-8-01~Z-8-07 개별 생성(~56회) 대신 Z-8-ALT를 사용한다. `tilePositions` Vector2Int[] 배열은 set_property로 불안정 (CLAUDE.md 제한사항).
...
**Z-8 합계(ALT 적용)**: ~3회  
~~개별 실행 시: ~56회 (tilePositions 배열 별도 시 훨씬 초과)~~

---

### Z-8-ALT: Editor 스크립트를 통한 ZoneData SO 일괄 생성 (**기본 경로**)

**[기본 경로]** Z-8-01~Z-8-07 대신 이 절차를 실행한다.

```
create_script
  path: "Assets/_Project/Editor/CreateZoneAssets.cs"
  content: |
    // Editor 전용: ZoneData SO Zone A~G 7개 일괄 생성
    // 구역별 수치(비용, 레벨 요건, 타일 수, 장애물 수):
    //   -> copied from docs/systems/farm-expansion.md 섹션 2.1 (DES-012 canonical)
    ...
    public static class CreateZoneAssets
    {
        ...
        {
            // Zone A~G 7개 ZoneData SO 생성
            // zoneId, displayName, unlockCost, requiredLevel, tilePositions[]:
            //   -> see docs/systems/farm-expansion.md 섹션 2.1 for 각 구역 수치
            // obstacles[]: -> see docs/systems/farm-expansion.md 섹션 3 for 장애물 목록
...
```
execute_menu_item
  menu: "SeedMind/Create Zone Assets"
```

- **MCP 호출**: 3회 (create_script 1 + execute_menu_item 1 + read_console 1)
- **Z-8-01~Z-8-07 대비 절감**: ~53회 (56 → 3)
