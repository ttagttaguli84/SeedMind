...
| ARC-029 | `docs/systems/fishing-architecture.md` 섹션 4A | 설계 완료 | FishingProficiency 클래스, 보정 메서드, FishingConfig 확장, 세이브 데이터 확장 |
...
| S-05 | `Scripts/Fishing/FishingSaveData.cs` | `FishingSaveData` (Serializable, fishingProficiencyXP/Level 포함) | `SeedMind.Fishing` | F-1 Phase 1 |
| S-06 | `Scripts/Fishing/FishingEvents.cs` | `FishingEvents` (static, OnProficiencyLevelUp 포함) | `SeedMind.Fishing` | F-1 Phase 2 |
| S-11 | `Scripts/Fishing/FishingProficiency.cs` | `FishingProficiency` (Plain C#, ARC-029) | `SeedMind.Fishing` | F-1 Phase 2 |
...
| S-12 | `Scripts/Fishing/FishingManager.cs` | `FishingManager` (MonoBehaviour, Singleton, ISaveable) | `SeedMind.Fishing` | F-1 Phase 4 |

(모든 경로 접두어: `Assets/_Project/`)

**컴파일 순서**: S-01~S-05 -> S-06~S-11 -> S-09~S-10 -> S-12. 각 Phase 사이에 Unity 컴파일 대기가 필요하다.

[RISK] S-12(FishingManager)은 SeedMind.Core, SeedMind.Farm, SeedMind.Level, SeedMind.Player에 대한 참조가 필요하므로, asmdef 의존성 누락 시 컴파일 실패한다.
...
create_script
  ...
  content: FishingSaveData (Serializable class)
    필드: totalCasts, totalCaught, totalFailed, caughtByFishId,
          rareFishCaught, maxStreak, currentStreak,
          fishingProficiencyXP, fishingProficiencyLevel  // ARC-029
...
create_script
  ...
  content: FishingEvents (static class)
    이벤트: OnFishCast (Action<FishingPoint>),
            ...
            OnProficiencyLevelUp (Action<int>),  // ARC-029: newLevel
...
### 4.8 F-8: 숙련도 시스템 (ARC-029)

**의존 태스크**: F-1 (스크립트 생성, S-11 포함), F-3 (FishingConfig SO)

#### F-8-01: FishingProficiency 스크립트 생성

```
create_script
  path: "Assets/_Project/Scripts/Fishing/FishingProficiency.cs"
  content: FishingProficiency (Plain C# class)
    필드: _currentXP (int), _currentLevel (int), _config (FishingConfig)
    프로퍼티: CurrentXP, CurrentLevel, XPToNextLevel, IsMaxLevel
    이벤트: OnLevelUp (Action<int>)
    메서드: AddXP(int), GetXPForCatch(FishRarity, bool),
            GetBiteDelayMultiplier(), GetRarityBonus(),
            GetTreasureChestBonus(), GetMaxFishQuality(),
            GetDoubleCatchChance(), GetEnergyCostReduction(),
            GetSaveData(), LoadSaveData(int, int)
  // -> see docs/systems/fishing-architecture.md 섹션 4A (ARC-029)
  // 모든 보정값은 FishingConfig SO 배열에서 조회
  // 수치 canonical -> see docs/systems/fishing-system.md 섹션 7.2~7.4
```

- **MCP 호출**: 1회

#### F-8-02: FishingConfig SO에 숙련도 필드 추가

```
modify_script
  path: "Assets/_Project/Scripts/Fishing/Data/FishingConfig.cs"
  action: 숙련도 설정 필드 추가
    proficiencyXPThresholds: int[]
      // -> see docs/systems/fishing-system.md 섹션 7.2 (canonical)
    proficiencyMaxLevel: int
      // -> see docs/systems/fishing-system.md 섹션 7.2 (canonical)
    catchXPByRarity: int[]
      // -> see docs/systems/fishing-system.md 섹션 7.3 (canonical)
    giantXPMultiplier: float
      // -> see docs/systems/fishing-system.md 섹션 7.3 (canonical)
    treasureChestXP: int
      // -> see docs/systems/fishing-system.md 섹션 7.3 (canonical)
    failXP: int
      // -> see docs/systems/fishing-system.md 섹션 7.3 (canonical)
    biteDelayMultiplierByLevel: float[]
      // -> see docs/systems/fishing-system.md 섹션 7.4 (canonical)
    rarityBonusByLevel: float[]
      // -> see docs/systems/fishing-system.md 섹션 7.4 (canonical)
    treasureBonusByLevel: float[]
      // -> see docs/systems/fishing-system.md 섹션 7.4 (canonical)
    doubleCatchChanceByLevel: float[]
      // -> see docs/systems/fishing-system.md 섹션 7.4 (canonical)
    maxQualityByLevel: CropQuality[]
      // -> see docs/systems/fishing-system.md 섹션 7.2 (canonical)
    energyCostReductionByLevel: int[]
      // -> see docs/systems/fishing-system.md 섹션 7.2 (canonical)
  // -> see docs/systems/fishing-architecture.md 섹션 4A.3 (ARC-029)
```

- **MCP 호출**: 1회

#### F-8-03: FishingConfig_Default SO 에셋에 숙련도 값 설정

```
set_property
  target: "Assets/_Project/Data/Fish/SO_FishingConfig.asset"
  // 에셋 경로는 F-3-01에서 생성한 경로를 따른다 (-> see F-3-01)
  // proficiencyXPThresholds: (→ see docs/systems/fishing-system.md 섹션 7.2)
  // proficiencyMaxLevel: (→ see docs/systems/fishing-system.md 섹션 7.2)
  // catchXPByRarity: (→ see docs/systems/fishing-system.md 섹션 7.3)
  // giantXPMultiplier: (→ see docs/systems/fishing-system.md 섹션 7.3)
  // treasureChestXP: (→ see docs/systems/fishing-system.md 섹션 7.3)
  // failXP: (→ see docs/systems/fishing-system.md 섹션 7.3)
  // 레벨별 배열: (→ see docs/systems/fishing-system.md 섹션 7.4)
```

- **MCP 호출**: ~12회 (필드별 set_property)

#### F-8-04: FishingManager에 숙련도 통합

```
modify_script
  path: "Assets/_Project/Scripts/Fishing/FishingManager.cs"
  action:
    1) _proficiency: FishingProficiency 필드 추가
    2) Initialize()에서 _proficiency = new FishingProficiency(_fishingConfig) 초기화
    3) _proficiency.OnLevelUp += (level) => FishingEvents.OnProficiencyLevelUp?.Invoke(level) 위임
    4) CalculateBiteDelay()에 _proficiency.GetBiteDelayMultiplier() 적용
    5) SelectRandomFish()에 _proficiency.GetRarityBonus() 적용
    6) CompleteFishing(true)에 품질 제한 + 더블 캐치 + 숙련도 XP 부여 추가
    7) CompleteFishing(false)에 실패 XP 부여 추가
    8) TryStartFishing()에 에너지 감소 보정 적용
    9) GetSaveData()에 fishingProficiencyXP/Level 추가
    10) LoadSaveData()에 _proficiency.LoadSaveData() 추가
  // -> see docs/systems/fishing-architecture.md 섹션 4A.7 (ARC-029)
```

- **MCP 호출**: 1회 (modify_script)
...
// 낚시 성공 시뮬레이션 x10 (Common)
execute_method  target: "FishingManager"  method: "SimulateFishingSuccess"  args: ["fish_carp"]
// (x10 반복)

get_console_logs
  filter: "FishingProficiency"
  // "FishingProficiency: 레벨 업! Lv.2" 로그 확인 (50 XP 도달 시)
  // 현재 XP = 50 (5 XP x 10회) -> Lv.2 달성

// 보정 효과 확인
execute_method  target: "FishingManager"  method: "DebugLogProficiencyBonuses"
  // BiteDelayMultiplier, RarityBonus, MaxQuality 등 로그 출력
...
get_console_logs
  filter: "FishingManager"
  // "FishingManager: Loaded -- proficiencyXP=50, proficiencyLevel=2" 확인

exit_play_mode
```

- **MCP 호출**: ~15회
- **검증**: Lv.2 레벨업 이벤트 발행, 보정값 적용, 세이브/로드 후 숙련도 XP/레벨 유지

**F-8 합계**: 1 + 1 + 12 + 1 + 15 = **~30회**
...
```
F-1 (스크립트 생성, S-11 FishingProficiency 포함)
  |
  ├── F-2 (FishData SO 에셋 15종) ──┐
  |                                  |
  ├── F-3 (FishingConfig SO 에셋) ──┼── F-8 (숙련도, ARC-029)
  |                                  |        |
  └── F-5 (기존 시스템 확장) ────────┤        |
                                     |        |
                       F-4 (씬 배치) ←┘       |
                         ...
                         ├── F-6 (연동 설정) ←┘
                         |
                         └── F-7 (통합 테스트) ← F-6, F-8 완료 후
```

**병렬 실행 가능 그룹**:
- F-2, F-3, F-5는 F-1 완료 후 병렬 실행 가능
- F-4는 F-2, F-3 완료 후 실행 (SO 에셋 참조 연결 필요)
- F-8은 F-1, F-3 완료 후 실행 (FishingProficiency.cs + FishingConfig SO 필요)
- F-6은 F-4, F-5 완료 후 실행
- F-7은 모든 태스크(F-6, F-8 포함) 완료 후에만 실행
...
| 문서 | 관련 내용 |
|------|----------|
| `docs/systems/fishing-architecture.md` (ARC-026, ARC-029) | 전체 아키텍처 설계, 클래스 다이어그램, 숙련도 아키텍처(섹션 4A), MCP Phase 요약 |
