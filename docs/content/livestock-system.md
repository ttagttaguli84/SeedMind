...
```
// 제안: progression-architecture.md XPSource enum 확장
public enum XPSource
{
    // 기존 (-> see docs/systems/progression-architecture.md 섹션 2.2)
    CropHarvest,
    ToolUse,
    FacilityBuild,
    FacilityProcess,
...
닭 x4 + 소 x1 초기 투자(8,700G, Zone E 제외) 회수: **67일** (약 2.4계절)
...
|---------|---------|-----------|----------|------|
| 최소 (닭 x2) | 7,100G | 50G/일 | 142일 (~5계절) | 보수적 진입 |
| 중간 (닭 x4 + 소 x1) | 12,700G | 130G/일 | 98일 (~3.5계절) | 권장 경로 |
| 적극 (닭 x4 + 소 x2 + 염소 x2) | 20,700G | 230G/일 | 90일 (~3.2계절) | 빠른 확장 |
...
치즈 공방 가공이 활성화되면 우유/염소젖의 부가가치가 높아진다 (예상 ~1.8x~2.0x). 이 경우:

| 시나리오 | 일일 순수익 (직판) | 일일 순수익 (가공 포함, 예상) | 증가율 |
|---------|-----------------|---------------------------|--------|
| 중간 (닭 x4 + 소 x1) | 130G/일 | ~180G/일 | +38% |
| 적극 (닭 x4 + 소 x2 + 염소 x2) | 230G/일 | ~340G/일 | +48% |

[OPEN] 치즈 공방 가공품 판매가가 미확정이므로 위 수치는 추정치. 치즈 공방 문서 작성 시 본 섹션의 수치를 갱신해야 한다.
...
| 문서 | 관련 내용 |
|------|----------|
| `docs/design.md` 섹션 4.6 | 시설 목록 -- 외양간/닭장 추가 필요 |
...
2. [RISK] **design.md 시설 테이블 미동기**: 외양간/닭장/치즈 공방이 `docs/design.md` 섹션 4.6 시설 목록에 포함되어 있지 않다. 본 문서 확정 후 design.md 갱신이 필요하다
