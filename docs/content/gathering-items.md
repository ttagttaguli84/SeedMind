...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | **봄나물 비빔밥** 재료 (달래 x2 + 봄나물 x1 -> 봄나물 비빔밥). 베이커리에서 제작. [OPEN] 레시피 상세는 processing-system.md에 추가 필요 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | **봄나물 비빔밥** 재료 (달래 x2 + 봄나물 x1). 베이커리에서 제작. [OPEN] 레시피 상세는 processing-system.md에 추가 필요 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | [OPEN] **꽃다발** 재료 후보 (제비꽃 x3 -> 봄 꽃다발). 가공소에서 제작. 레시피 신규 제안 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | **건조 버섯** 재료 (송이(봄) x2 -> 건조 버섯 x1). 가공소에서 제작. [OPEN] 레시피 상세는 processing-system.md에 추가 필요 |
...
| `maxStack` | 5 |
| 카테고리 | `Gathered` |
| **가공 연계** | [OPEN] **산삼주** 신규 레시피 제안 (산삼 x1 -> 산삼주 x1, 발효실, 판매가 ~500G, 발효 3일). Legendary 아이템의 고부가가치 가공 경로 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | **야생 베리잼** 재료 (산딸기 x5 -> 야생 베리잼 x1, 가공소, 판매가 ~25G). [OPEN] 레시피 상세는 processing-system.md에 추가 필요. gathering-system.md 섹션 9.1 예시와 정합 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | [OPEN] **쑥떡** 신규 레시피 제안 (쑥 x3 + 쌀 x1 -> 쑥떡 x1, 베이커리, 판매가 ~30G). 쌀 획득 경로는 fishing-system.md 섹션 8의 [OPEN] 항목과 연계 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | [OPEN] **연잎밥** 신규 레시피 제안 (연잎 x1 + 쌀 x2 -> 연잎밥 x1, 베이커리, 판매가 ~50G). 쌀 획득 경로 확정 필요 |
| **NPC 선물** | 하나: 보통 / 철수: 보통 / 목이: 좋아함 / 바람이: 보통 |
| **업그레이드 재료** | 해당 없음 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | **건조 버섯** 재료 (영지버섯 x2 -> 건조 영지 x1, 가공소, 판매가 ~120G). [OPEN] 레시피 상세는 processing-system.md에 추가 필요. 건조 영지는 일반 건조 버섯과 별개 가공품으로 제안 (Rare 원재료의 부가가치 반영) |
| **NPC 선물** | 하나: 좋아함 / 철수: 보통 / 목이: 좋아함 / 바람이: 좋아함 |
| **업그레이드 재료** | 해당 없음 |
...
| `maxStack` | 5 |
| 카테고리 | `Gathered` |
| **가공 연계** | [OPEN] **황금 연꽃차** 신규 레시피 제안 (황금 연꽃 x1 -> 황금 연꽃차 x1, 가공소, 판매가 ~600G, 가공 시간 2시간). Legendary 아이템의 고부가가치 가공 경로 |
| **NPC 선물** | 하나: 좋아함 / 철수: 좋아함 / 목이: 좋아함 / 바람이: 좋아함 (전원 좋아함 — Legendary 특전) |
| **업그레이드 재료** | 해당 없음 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | **도토리묵** 재료 (도토리 x5 -> 도토리묵 x1, 가공소, 판매가 ~20G). [OPEN] 레시피 상세는 processing-system.md에 추가 필요. gathering-system.md 섹션 9.1 예시와 정합 |
| **NPC 선물** | 하나: 보통 / 철수: 보통 / 목이: 좋아함 / 바람이: 보통 |
| **업그레이드 재료** | 해당 없음 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | **건조 버섯** 재료 (능이버섯 x3 -> 건조 버섯 x1, 가공소, 판매가 ~40G). [OPEN] 레시피 상세는 processing-system.md에 추가 필요. gathering-system.md 섹션 9.1 예시와 정합 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | **건조 버섯** 재료 (표고버섯(야생) x2 -> 건조 버섯 x1, 가공소, 판매가 ~60G). [OPEN] 레시피 상세는 processing-system.md에 추가 필요 |
| **NPC 선물** | 하나: 좋아함 / 철수: 보통 / 목이: 보통 / 바람이: 좋아함 |
| **업그레이드 재료** | 해당 없음 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | **머루 와인** 재료 (머루 x5 -> 머루 와인 x1, 발효실, 판매가 ~70G, 발효 2일). [OPEN] 레시피 상세는 processing-system.md에 추가 필요. gathering-system.md 섹션 9.1 예시와 정합 |
| **NPC 선물** | 하나: 좋아함 / 철수: 보통 / 목이: 보통 / 바람이: 좋아함 |
| **업그레이드 재료** | 해당 없음 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | [OPEN] **송이 구이** 신규 레시피 제안 (송이버섯 x1 -> 송이 구이 x1, 베이커리, 판매가 ~160G, 가공 시간 30분). Rare 버섯의 고급 요리 경로 |
...
| `maxStack` | 5 |
| 카테고리 | `Gathered` |
| **가공 연계** | [OPEN] **천년 영지차** 신규 레시피 제안 (천년 영지 x1 -> 천년 영지차 x1, 가공소, 판매가 ~700G, 가공 시간 3시간). Legendary 아이템 최고 부가가치 가공 |
| **NPC 선물** | 하나: 좋아함 / 철수: 좋아함 / 목이: 좋아함 / 바람이: 좋아함 (전원 좋아함 — Legendary 특전) |
| **업그레이드 재료** | 해당 없음 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | [OPEN] **나무껍질 차** 신규 레시피 제안 (겨울 나무껍질 x5 -> 나무껍질 차 x1, 가공소, 판매가 ~15G). 겨울 채집의 소소한 가공 경로. 수익 미미하나 겨울 가공 활동 콘텐츠 |
| **NPC 선물** | 하나: 보통 / 철수: 보통 / 목이: 좋아함 / 바람이: 보통 |
| **업그레이드 재료** | 해당 없음 |
...
| `maxStack` | 30 |
| 카테고리 | `Gathered` |
| **가공 연계** | [OPEN] **동충하초 환** 신규 레시피 제안 (동충하초 x2 -> 동충하초 환 x1, 가공소, 판매가 ~250G, 가공 시간 2시간). 겨울 Rare 아이템의 고부가가치 가공 |
| **NPC 선물** | 하나: 좋아함 / 철수: 좋아함 / 목이: 보통 / 바람이: 좋아함 |
| **업그레이드 재료** | 해당 없음 |
...
| **가공 연계** | 없음 (업그레이드 재료 전용) |
| **NPC 선물** | 하나: 보통 / 철수: 좋아함 / 목이: 보통 / 바람이: 보통 |
| **업그레이드 재료** | [OPEN] 도구 업그레이드 대체 재료 후보. 현재 도구 업그레이드는 철 조각(`iron_scrap`, 상점 구매)을 사용 (-> see `docs/systems/tool-upgrade.md` 섹션 2.2). 철 광석을 철 조각의 대체/추가 재료로 활용할지 결정 필요. 예: 철 광석 x2를 가공소에서 철 조각 x1로 정제하는 경로 |
...
| `maxStack` | 30 |
| 카테고리 | `Mineral` |
| **가공 연계** | [OPEN] **수정 장식품** 신규 레시피 제안 (수정 원석 x2 -> 수정 장식품 x1, 가공소, 판매가 ~200G). 고급 장식 아이템 가공 경로 |
| **NPC 선물** | 하나: 좋아함 / 철수: 보통 / 목이: 좋아함 / 바람이: 좋아함 |
| **업그레이드 재료** | 해당 없음 |
...
| `maxStack` | 10 |
| 카테고리 | `Mineral` |
| **가공 연계** | [OPEN] **자수정 목걸이** 신규 레시피 제안 (자수정 x1 + 금 광석 x1 -> 자수정 목걸이 x1, 가공소, 판매가 ~400G). Legendary 광물의 최고급 장신구 가공 경로 |
...
### 9.1 기존 가공 레시피 연계 (gathering-system.md 섹션 9.1 기반)

아래 레시피는 gathering-system.md 섹션 9.1에서 예시로 제시된 것을 구체화한 것이다. 실제 레시피 정의는 processing-system.md에 추가되어야 한다.

| 가공품명 | 재료 | 가공소 | 예상 판매가 | 가공 시간 | 비고 |
|----------|------|--------|:----------:|:---------:|------|
| 야생 베리잼 | 산딸기 x5 | 가공소 | ~25G | 1시간 | Common 재료 대량 투입 |
| 건조 버섯 (능이) | 능이버섯 x3 | 가공소 | ~40G | 2시간 | Common 버섯 가공 |
| 건조 버섯 (표고) | 표고버섯(야생) x2 | 가공소 | ~60G | 2시간 | Uncommon 버섯 가공 |
| 도토리묵 | 도토리 x5 | 가공소 | ~20G | 1시간 | Common 재료 대량 투입 |
| 머루 와인 | 머루 x5 | 발효실 | ~70G | 2일 | 발효 시간 투자형 |
| 봄나물 비빔밥 | 달래 x2 + 봄나물 x1 | 베이커리 | ~50G | 30분 | 복합 재료 요리 |

[OPEN] 위 레시피 6종의 정확한 사양(해금 조건, 판매가 공식, 가공 시간)을 processing-system.md에 추가해야 한다.

### 9.2 신규 가공 레시피 제안

본 문서에서 새롭게 제안하는 가공 레시피. 모두 [OPEN] 상태이며, processing-system.md에서 채택 여부를 결정한다.

| 가공품명 | 재료 | 가공소 | 예상 판매가 | 가공 시간 | 설계 의도 |
|----------|------|--------|:----------:|:---------:|-----------|
| 건조 영지 | 영지버섯 x2 | 가공소 | ~120G | 2시간 | Rare 버섯 별도 가공품으로 차별화 |
| 건조 버섯 (송이/봄) | 송이(봄) x2 | 가공소 | ~90G | 2시간 | Rare 버섯 가공 |
| 쑥떡 | 쑥 x3 + 쌀 x1 | 베이커리 | ~30G | 30분 | 쌀 획득 경로 확정 필요 |
| 연잎밥 | 연잎 x1 + 쌀 x2 | 베이커리 | ~50G | 30분 | Zone F 연계 요리 |
| 송이 구이 | 송이버섯 x1 | 베이커리 | ~160G | 30분 | Rare 고급 요리 |
| 산삼주 | 산삼 x1 | 발효실 | ~500G | 3일 | Legendary 최고급 가공 |
| 황금 연꽃차 | 황금 연꽃 x1 | 가공소 | ~600G | 2시간 | Legendary 최고급 가공 |
| 천년 영지차 | 천년 영지 x1 | 가공소 | ~700G | 3시간 | Legendary 최고 부가가치 |
| 동충하초 환 | 동충하초 x2 | 가공소 | ~250G | 2시간 | 겨울 Rare 가공 |
| 나무껍질 차 | 겨울 나무껍질 x5 | 가공소 | ~15G | 30분 | 겨울 소소한 가공 |
| 수정 장식품 | 수정 원석 x2 | 가공소 | ~200G | 1시간 | 고급 장식 가공 |
| 자수정 목걸이 | 자수정 x1 + 금 광석 x1 | 가공소 | ~400G | 2시간 | 최고급 장신구 |
| 꽃다발 (봄) | 제비꽃 x3 | 가공소 | ~60G | 30분 | NPC 선물용 |

[OPEN] 위 13종 신규 레시피 중 어떤 것을 채택할지 결정 필요. 특히:
1. 쌀(`rice`) 획득 경로 미확정 -> 쑥떡, 연잎밥은 쌀 경로 확정 후에만 채택 가능
2. Legendary 가공품(산삼주, 황금 연꽃차, 천년 영지차)의 판매가가 과도할 수 있음. 밸런스 시트 검증 필요
3. 장신구(수정 장식품, 자수정 목걸이)는 실용적 기능 없이 판매/선물 전용. 게임 내 역할 충분한지 검토

### 9.3 가공 부가가치 분석

| 원재료 | 원재료 판매가 합계 | 가공품 판매가 | 부가가치 비율 | 평가 |
|--------|:-----------------:|:------------:|:------------:|------|
| 산딸기 x5 | 50G | ~25G | 0.50x | 손해 — 재조정 필요 |
| 능이버섯 x3 | 45G | ~40G | 0.89x | 손해 — 재조정 필요 |
| 표고(야생) x2 | 60G | ~60G | 1.00x | 본전 — 약간 상향 필요 |
| 도토리 x5 | 40G | ~20G | 0.50x | 손해 — 재조정 필요 |
| 머루 x5 | 110G | ~70G | 0.64x | 손해 — 재조정 필요 |
| 달래x2+봄나물x1 | 44G | ~50G | 1.14x | 적정 |
| 산삼 x1 | 200G | ~500G | 2.50x | 적정 (Legendary + 발효 3일) |
| 황금 연꽃 x1 | 250G | ~600G | 2.40x | 적정 (Legendary) |
| 천년 영지 x1 | 300G | ~700G | 2.33x | 적정 (Legendary) |

[RISK] gathering-system.md 섹션 9.1에서 제시한 예시 가공 가격(야생 베리잼 ~25G, 도토리묵 ~20G)은 원재료 투입량 대비 손해가 발생한다. 이는 가공의 매력을 크게 떨어뜨린다. 두 가지 해결 방안:
1. **재료 투입량 하향**: 산딸기 x3 -> 야생 베리잼 (원재료 30G < 가공 25G -> 여전히 손해). 근본적으로 가공 판매가를 상향해야 함.
2. **가공 판매가 상향**: 야생 베리잼 65G, 건조 버섯(능이) 55G, 도토리묵 50G 등으로 조정. 부가가치 비율 1.3x~1.5x가 되도록 재설정.

본 문서에서는 방향만 제시하며, 최종 가격은 processing-system.md에서 확정한다.
...
### 10.2 도구 업그레이드 재료 (현재 상태)

현재 도구 업그레이드는 상점 구매 재료(철 조각, 정제 강철)를 사용하며, 채집 광물은 사용하지 않는다 (-> see `docs/systems/tool-upgrade.md` 섹션 2.2).

[OPEN] 철 광석(`gather_iron_ore`)을 도구 업그레이드 대체 재료로 활용하는 경로 도입 여부:
- **방안 A**: 철 광석 x2 -> 가공소에서 철 조각 x1로 정제. "직접 채집으로 업그레이드 비용 절감" 선택지 제공
- **방안 B**: 도구 업그레이드 재료를 철 조각에서 철 광석으로 완전 대체. 채집-도구 업그레이드 직접 연결
- **방안 C**: 현행 유지 (상점 구매만). 채집 광물은 낚싯대 전용

채집-낚시-도구 간 교차 의존성 설계 의도와 스코프를 고려하여 결정 필요.
...
| `docs/systems/tool-upgrade.md` 섹션 2.2 | 본 문서 섹션 10.2 | 도구 업그레이드 재료 (철 광석 연계 [OPEN]) |
...
2. [OPEN] **채집물 가공 레시피 6종 상세**: processing-system.md에 야생 베리잼, 건조 버섯, 도토리묵, 머루 와인, 봄나물 비빔밥 + 건조 영지 레시피를 정식으로 추가해야 한다.
3. [OPEN] **신규 가공 레시피 13종 채택 여부**: 본 문서에서 제안한 신규 레시피 중 어떤 것을 processing-system.md에 추가할지 결정.
4. [OPEN] **가공 부가가치 재조정**: gathering-system.md 섹션 9.1의 예시 가격이 원재료 투입 대비 손해를 유발. 가공 판매가 상향 또는 재료 투입량 하향 필요.
5. [OPEN] **철 광석의 도구 업그레이드 재료 활용**: 방안 A(가공소 정제)/B(완전 대체)/C(현행 유지) 중 선택.
6. [OPEN] **쌀(`rice`) 획득 경로**: 쑥떡, 연잎밥 레시피는 쌀이 필요. fishing-system.md 섹션 8 [OPEN] 항목과 연계하여 확정 필요.
...
1. [RISK] **가공 부가가치 역전**: 현재 gathering-system.md 예시 가격 기준으로 일부 가공품이 원재료 직접 판매보다 손해. 플레이어가 "가공하면 오히려 손해"라고 느끼면 가공 시스템 전체의 매력이 하락한다. 섹션 9.3 참조.
2. [RISK] **Legendary 가공품 가격 과다**: 산삼주(500G), 황금 연꽃차(600G), 천년 영지차(700G)가 게임 경제에 미치는 영향. Legendary 아이템 자체가 극희소(출현율 2~6%, 특수 조건 필요)이므로 단기 경제 파괴 위험은 낮으나, 장기적으로 축적 판매 시 밸런스 검증 필요.
