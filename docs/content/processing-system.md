...
이 문서는 SeedMind의 가공/요리 시스템에 등장하는 **전체 레시피, 가공소 유형, 연료 규칙, 배치/가동 규칙, 가공품 퀄리티 연동**을 상세히 기술한다. 기존 단일 가공소(`building_processing`)의 잼/주스/절임 레시피를 기반으로, **특화 가공소 3종**(제분소, 발효실, 베이커리)을 신규 도입하여 가공 시스템의 깊이를 확장한다.
...
|--------|---------|------|----------|------|----------|
| 가공소 (일반) | `building_processing` | (-> see `docs/design.md` 섹션 4.6) | (-> see `docs/design.md` 섹션 4.6) | 잼, 주스, 절임, 생선 가공(구운/훈제/스튜) | 21종 |
| 제분소 | `building_mill` | 레벨 5 | 1,500G | 곡물/작물 -> 가루/분말 | 4종 |
| 발효실 | `building_fermentation` | 레벨 8 | 4,000G | 과일/채소 -> 와인, 식초, 된장 등 | 5종 |
| 베이커리 | `building_bakery` | 레벨 9 | 5,000G | 가공품 + 가공품 -> 고급 요리, 생선 가공(초밥/파이) | 7종 |

**총 레시피 수: 37종** (일반 가공소 21종[작물 가공 18 + 생선 가공소 분 3] + 제분소 4종 + 발효실 5종 + 베이커리 7종[요리 5 + 생선 베이커리 분 2])
...
**설계 의도**: 연료 시스템은 베이커리에만 적용한다. 가공소(일반)와 제분소, 발효실은 연료 없이 운영하여 진입 장벽을 낮추고, 최상위 시설인 베이커리만 추가 비용 요소를 가져 엔드게임 복잡도를 높인다.
...
**설계 의도**: 가공소(일반)만 슬롯 확장이 가능하다. 특화 가공소 3종은 슬롯이 고정되어, "어떤 레시피를 슬롯에 넣을지"의 선택을 강제한다. 발효실과 베이커리는 2슬롯으로 시작하여 동시에 2종의 레시피를 진행할 수 있지만, 24시간짜리 와인과 12시간짜리 식초 중 어느 쪽에 슬롯을 할당할지가 전략적 판단이다.
...
**가공 시간 단축**: 가공소(일반)는 업그레이드를 통해 가공 시간을 단축할 수 있다 (-> see `docs/content/facilities.md` 섹션 6.6). 특화 가공소 3종은 업그레이드 시스템이 없으며 가공 시간이 고정이다.
...
현재 `docs/design.md` 섹션 4.6 시설 목록에는 가공소 1종만 존재한다. 특화 가공소 3종을 추가하려면 design.md 업데이트가 필요하다.
...
[OPEN] design.md 섹션 4.6에 특화 가공소 3종을 반영해야 한다. 이 업데이트는 본 문서 승인 후 별도 작업으로 수행한다.
...
- BAL-004: 가공품 ROI 밸런스 분석 (32종 레시피 수익성, 가공 체인 효율, 시간 대비 수익)

---

## 9. Open Questions

1. **[OPEN]** 특화 가공소 3종(제분소, 발효실, 베이커리)을 `docs/design.md` 섹션 4.6에 반영하는 작업이 필요하다. 현재 design.md에는 가공소 1종만 기재되어 있다.
...
4. **[RISK]** 특화 가공소 3종의 건설 비용 합계(1,500G + 4,000G + 5,000G = 10,500G)가 기존 가공소 풀 업그레이드(18,500G)와 합치면 총 29,000G. 게임 전체 골드 유통량 대비 이 투자가 합리적인지 BAL-004에서 검증 필요.
