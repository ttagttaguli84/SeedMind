...
> 작성: Claude Code (Opus) | 2026-04-06 | FIX-083 채집물 가공 레시피 추가: 2026-04-07
...
|--------|---------|------|----------|------|----------|
| 가공소 (일반) | `building_processing` | (-> see `docs/design.md` 섹션 4.6) | (-> see `docs/design.md` 섹션 4.6) | 잼, 주스, 절임, 생선 가공(구운/훈제/스튜), **채집물 가공** | 30종 |
...
**총 레시피 수: 55종** (일반 가공소 30종[작물 가공 18 + 생선 가공소 분 3 + 채집물 9] + 제분소 4종 + 발효실 7종[작물 5 + 채집물 2] + 베이커리 9종[요리 5 + 생선 베이커리 분 2 + 채집물 2] + 치즈 공방 5종)
...
|----------|------|--------|-------------|----------|----------|------|----------|
| `recipe_gather12` | 달래 (`gather_wild_garlic`) x2 + 봄나물 (`gather_spring_herb`) x1 | 봄나물 비빔밥 | `item_spring_herb_bibimbap` | 30G | 30분 | 장작 1 | 베이커리 건설 + 채집 숙련도 Lv.2 |
| `recipe_gather13` | 송이버섯 (`gather_pine_mushroom`) x1 | 송이 구이 | `item_grilled_pine_mushroom` | 55G | 30분 | 장작 1 | 베이커리 건설 + 채집 숙련도 Lv.4 |

**판매가 근거**:

| 레시피 | 원재료 총 직판가 | 판매가 | 배수 | 설계 의도 |
|--------|:---------------:|:------:|:----:|-----------|
| 봄나물 비빔밥 | 18G (달래 5Gx2 + 봄나물 8Gx1) | 30G | 1.67x | 봄 채집 복합 재료 요리. 연료 30G 추가 비용 감안 시 실질 손익분기에 가까움 |
| 송이 구이 | 32G (32G x1) | 55G | 1.72x | Rare 버섯의 간편 고급 요리. 연료 30G 추가 비용 감안 시 실질 배수 ~0.78x로 연료비 대비 미미하나, NPC 선물 가치가 높음 |

**설계 의도**: 베이커리 채집물 레시피는 기존 베이커리 레시피(2.0~2.5x)보다 낮은 배수를 적용한다. 봄나물 비빔밥은 봄 채집 경험의 마무리 요리로, 수익성보다 "채집물로 요리를 만드는" 경험 자체가 목적이다. 송이 구이는 가을 Rare 송이버섯의 소비 경로로, 순수 판매 수익보다 NPC 선물 적합도(전 NPC 좋아함 예상)와 이벤트 활용이 주된 가치다.

[OPEN] 봄나물 비빔밥과 송이 구이의 연료 비용(장작 30G)을 감안하면 실질 수익이 매우 낮다. 베이커리 채집물 레시피에는 연료를 면제하거나, 판매가를 소폭 상향(비빔밥 40G, 송이 구이 65G)하는 방안을 BAL-004에서 검토할 필요가 있다.
...
| 54 | 베이커리 | `recipe_gather12` | 봄나물 비빔밥 | 30G |
| 55 | 베이커리 | `recipe_gather13` | 송이 구이 | 55G |
...
- BAL-004: 가공품 ROI 밸런스 분석 (55종 레시피 수익성, 가공 체인 효율, 시간 대비 수익 — 채집물 13종 포함)
