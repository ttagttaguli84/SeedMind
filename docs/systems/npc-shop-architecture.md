...
> **참고**: 아래 정의는 초기 버전이다. CON-008 확장 이후의 최신 버전은 **섹션 9.4** (TravelingMerchantSaveData 6필드) 및 **섹션 13.2** (NPCSaveData 통합)을 참조한다.
...
GenerateStock(playerLevel, currentSeason) 확장:
    ...
    4. 가중 랜덤 선택 (minItemCount ~ maxItemCount)
    ...
    8. _currentStock에 저장, _recentItems 갱신
```
...
PATTERN-005 준수: JSON 스키마와 C# 클래스의 필드 6개 (isPresent, randomSeed, currentStockItemIds, currentStockQuantities, currentStockPrices, recentItemIds)가 동일하다.

> **주의**: 섹션 7.2의 기존 TravelingMerchantSaveData(4필드)를 이 확장 버전(6필드)으로 교체한다. 기존 필드에 `currentStockPrices`, `recentItemIds` 2개가 추가되었다.
