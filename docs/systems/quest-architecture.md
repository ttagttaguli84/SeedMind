...
| 확장성 | 새 목표 타입 추가 시 ObjectiveType enum + 이벤트 구독 핸들러 1개만 추가 |
...
- [RISK] **이벤트 구독 누락**: QuestTracker가 12종 ObjectiveType을 추적하기 위해 9개 이상의 외부 이벤트를 구독해야 한다. 하나라도 누락되면 해당 목표 타입이 동작하지 않는다. SubscribeAll()/UnsubscribeAll()에서 모든 이벤트를 명시적으로 나열하고, 테스트 시나리오에서 각 ObjectiveType별 검증이 필요하다.
