...
Pseudocode: GetSupplyMultiplier(string itemId, SupplyCategory category)
    ...
    // Crop: 0.02, AnimalProduct: 0.008, Fish: 0.01, Forage: 0.008 (→ see economy-system.md 섹션 7.3)

    multiplier = max(supplyParams.minSupplyMultiplier, 1.0 - supplyPenalty)
    // Crop: 최저 0.70, AnimalProduct: 최저 0.85, Fish: 최저 0.80, Forage: 최저 0.85 (→ see economy-system.md 섹션 7.3)
...
namespace SeedMind   // 최상위 네임스페이스 — ItemType, CropQuality와 동일 레벨
{
    ...
    public enum HarvestOrigin
    {
        ...
        Fishing    = 3,   // 낚시 포획물 (→ see docs/systems/fishing-architecture.md, FIX-049)
        Gathering  = 4,   // 야생 채집물 (→ see docs/systems/gathering-system.md, ARC-031)
...
Pseudocode: GetGreenhouseMultiplier(HarvestOrigin origin, CropData cropData, Season currentSeason)
    ...
    if origin == HarvestOrigin.Fishing:
        return 1.0  // 낚시 포획물은 온실 보정 적용 대상 외 (→ see FIX-049)

    if origin == HarvestOrigin.Gathering:
        return 1.0  // 야생 채집물은 온실 보정 적용 대상 외 (→ see ARC-031)
...
| `origin == Fishing` | 1.0 | 낚시 포획물 — 온실 보정 적용 대상 외 (FIX-049) |
| `origin == Gathering` | 1.0 | 야생 채집물 — 온실 보정 적용 대상 외 (ARC-031) |
...
[RESOLVED-FIX-076] 야생 채집(forageable) 시스템 추가에 따라 `HarvestOrigin.Gathering = 4` 및 `SupplyCategory.Forage = 4` 추가 완료 (→ see docs/systems/gathering-system.md, ARC-031).
...
| Forage | 25 | 0.008 | 0.85 | 야생 채집물: 낮은 수급 민감도, 채집 포인트 리스폰 2~3일 반영 (→ see docs/systems/gathering-system.md, ARC-031) |
...
**파급 영향**:
- `EconomyConfig.categorySupplyParams` 배열에 5개 SupplyParams 원소를 MCP로 설정 (-> see 섹션 4.1)
...
namespace SeedMind.Economy.Data
{
    ...
    public class EconomyConfig : ScriptableObject
    {
        ...
        public SupplyParams[] categorySupplyParams = new SupplyParams[5];
        // indexed by SupplyCategory (0=Crop, 1=AnimalProduct, 2=Fish, 3=ProcessedGoods, 4=Forage)
...
namespace SeedMind.Economy.Data
{
    ...
    public enum PriceCategory
    {
        ...
        Forage,     // 채집물 (판매 전용, → see docs/systems/gathering-system.md, ARC-031)
    ...
    public enum SupplyCategory
    {
        ...
        Forage          = 4   // 채집물: 야생 수급, 낮은 민감도 (→ see docs/systems/gathering-system.md, ARC-031)
    }
}
```
