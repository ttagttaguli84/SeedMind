...
│  unlockCost: int  │          │      void                │
│  prerequisiteZoneId:string│  │  + DeactivateZoneTiles(  │
│  tilePositions:   │          │      Vector2Int[] pos):  │
│    Vector2Int[]   │          │      void                │
│  zoneType: ZoneType│         │  + GetTile(pos): FarmTile│
│  obstacleMap:     │          └──────────────────────────┘
│    ObstacleEntry[]│
│  sortOrder: int   │
└───────────────────┘
...
namespace SeedMind.Farm
{
    ...
    public class ZoneData : ScriptableObject
    {
        ...
        public string prerequisiteZoneId; // 선행 구역 ID (null/empty이면 선행 없음) (→ see docs/systems/farm-expansion.md 섹션 2.1)
...
private bool CanToolClear(ObstacleType obstacle, ToolType tool, int toolTier)
{
    ...
    // ObstacleType 값 → see 섹션 2.3 (Weed/SmallRock/LargeRock/Stump/SmallTree/LargeTree/Bush)
    switch (obstacle)
    {
...
Step A-5: 장애물 프리팹 Placeholder 생성 (7종, → see ObstacleType 섹션 2.3)
          → PFB_Obstacle_Weed (녹색 Quad, scale 0.3)
          → PFB_Obstacle_SmallRock (회색 Sphere, scale 0.4)
          → PFB_Obstacle_LargeRock (회색 Sphere, scale 0.8)
          → PFB_Obstacle_Stump (갈색 Cylinder, scale 0.5)
          → PFB_Obstacle_SmallTree (갈색 Cylinder, scale 0.3 x 0.8)
          → PFB_Obstacle_LargeTree (갈색 Cylinder, scale 0.5 x 1.5)
          → PFB_Obstacle_Bush (진녹색 Sphere, scale 0.5)
...
Step C-2: FarmGrid.InitializeFullGrid(16, 16) 호출 확인
          ...
          → 초기 구역(zone_home) 타일만 active, 나머지 inactive

Step C-3: Play Mode 통합 테스트
          → TryUnlockZone("zone_south_plain") 호출 (충분한 골드/레벨 설정 후)
          → 새 타일 활성화 확인 (SetActive true)
          → ZoneEvents.OnZoneUnlocked 이벤트 발행 확인
          → Console 로그: "Zone zone_south_plain unlocked, activated N tiles"
```
...
Step E-3: GameSaveData에 farmZones 필드 추가
          ...
          → Console 로그: "FarmZoneManager loaded, unlocked zones: N/7"
```
