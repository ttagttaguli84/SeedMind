...
| 데이터 종류 | 참조처 |
|------------|--------|
| 업적별 목표 수치 (수확 N회, 골드 N 달성 등) | 향후 `docs/content/achievements.md` (미작성) |
| 업적 보상 수치 (골드, XP) | `docs/balance/progression-curve.md` |
...
| 1-2 | AchievementConditionType.cs | enum | SeedMind.Achievement.Data | 14개 값 |
...
(-> see 향후 `docs/content/achievements.md` for 업적 목록 및 수치)
...
- [OPEN] 업적 총 개수: 카테고리별 목표 수 및 총 개수는 향후 `docs/content/achievements.md`에서 확정. 현재 디자인 문서 기준 30개 (→ see `docs/systems/achievement-system.md` 섹션 1).
- [OPEN] 업적 보상 수치: 카테고리/난이도별 골드/XP 보상 기준을 `docs/balance/progression-curve.md`에 추가해야 한다.
