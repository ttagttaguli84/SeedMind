...
#pragma warning disable CS0618 // GetInstanceID obsolete in Unity 6; int-based ID migration deferred
namespace MCPForUnity.Editor.Tools
{
    ...
    public static class ManageScene
    {
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            // If name looks like a full path (contains '/' or ends with .unity),
            // extract it into path + bare name so downstream logic works correctly.
            if (!string.IsNullOrEmpty(name) && (name.Contains('/') || name.Contains('\\') || name.EndsWith(".unity", StringComparison.OrdinalIgnoreCase)))
            {
                string normalized = AssetPathUtility.NormalizeSeparators(name);
                ...
                path = path ?? Path.GetDirectoryName(normalized);
