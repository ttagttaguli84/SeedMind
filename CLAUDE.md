...
## 핵심 규칙
1. Language: 모든 출력 한국어. 코드/변수명은 영어.
2. Document-only phase: 코드 파일 생성 금지. `docs/` 하위 마크다운만 작성.
3. 매 작업 후: 문서 작성 → `/review` → 수정 → commit → push.
4. 주기적: `/self-improve` (3+ 동일 이슈 발견 시, 또는 10+ TODO 완료 시).

## 에이전트 체계
| Agent | Role |
|-------|------|
| designer | 게임 디자인 확장/심화 |
| architect | 기술 아키텍처 설계 |
| reviewer | 문서 일관성/완전성 검증 |
| self-improve | 반복 패턴 감지 → 규칙/커맨드 개선 |
